...
......................................................................*)
  ...
  let add_listener (evt : 'a event) (listener : 'a -> unit) : id =
    let n = new_id () in
    let new_waiter = {id = n; action = listener} in
    evt := (new_waiter :: !evt); n;;
...
let ids = List.map (WEvent.add_listener newswire) [fakeNewsNetwork; buzzFake] ;;
...
List.iter (WEvent.remove_listener newswire) ids ;;
...
let receive_report (s : string) : unit = 
  let ids = List.map (WEvent.add_listener publish) 
    [(fun () -> fakeNewsNetwork s); (fun () -> buzzFake s)] in () ;;
...
let report_id = WEvent.add_listener newswire receive_report ;;
...
List.map (WEvent.fire_event newswire) [h4; h5; h6] ;;
